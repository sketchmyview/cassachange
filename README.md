<p style="background-color: #2b2b2b; padding: 20px; display: inline-block;">
  <img src="https://www.cassachange.com/github_cassachange.png" alt="cassachange" width="200"/>
</p>
...
30. [Complementary Tools](#30-complementary-tools)
...
| monitor — live table health snapshots | ✔ enterprise | ✗ | ✗ |
| analyse — 23-rule schema analyser | ✔ enterprise | ✗ | ✗ |
| keyspace schema diff | ✔ enterprise | ✗ | ✗ |
| migration script from diff | ✔ enterprise | ✗ | ✗ |
| schema-aware test data seeder | ✔ enterprise | ✗ | ✗ |
| CI/CD analytics HTML report | ✔ enterprise | ✗ | ✗ |
| schema contract enforcement | ✔ enterprise | ✗ | ✗ |
| growth projection engine | ✔ enterprise | ✗ | ✗ |
...
## 30. Complementary Tools

Seven enterprise grade features that integrate with the core `cassachange` command. Each follows the same pattern — same `cassachange.yml`, same `--profile` flag, same CLI conventions. Install only what you need.

---

### cassachange-monitor

Captures live table health snapshots across 5 probe categories. Writes results to `monitor_snapshots` in `history_keyspace` for trend tracking.

**Probes:**

| Probe | Metric |
|---|---|
| `row_count` | Total live rows per table |
| `partition_size` | Mean and max partition size in MB |
| `collection_size` | Average element count per collection column |
| `ttl_drift` | Live row count on tables with TTL (rising = TTL not expiring) |
| `tombstone_ratio` | Estimated tombstone ratio per table |

```bash
# Snapshot all tables in keyspace
cassachange monitor --profile prod

# Single table
cassachange monitor --profile prod --table user_events

# JSON output
cassachange monitor --profile prod --json

# Per-table JSON files (one file per table)
cassachange monitor --profile prod --json-dir ./snapshots/
```

Config keys in `cassachange.yml`:

```yaml
monitor_verbose: false
monitor_json: false
monitor_json_dir: null        # write per-table JSON files here
monitor_skip: []              # table names to skip
monitor_sample_limit: 100000  # row sample limit per table
```

---

### cassachange-analyser

23-rule schema analyser. Scores each keyspace 0–100 and lists findings by severity.

**Rule categories:**

| Category | Rules | Examples |
|---|---|---|
| Partition Key Quality | PK001–PK004 | Low-cardinality PK, timestamp-only PK, date-first composite |
| Query Safety | QS001–QS004 | ALLOW FILTERING risk, too many clustering columns, text clustering |
| Table Health | TH001–TH005 | Ephemeral table no TTL, unbounded collection, counter mixed |
| Schema Anti-patterns | SA001–SA006 | Integer ID as PK, no temporal anchor, reserved keyword columns |
| Index Recommendations | IR001–IR005 | Too many secondary indexes, high/low cardinality index targets |

```bash
# Analyse all tables
cassachange analyse --profile prod

# Skip specific rules
cassachange analyse --profile prod --skip QS004,IR004
...
Exit codes: `0` = all pass / `1` = warnings / `2` = critical findings.

---

### cassachange-diff

Compares two keyspace schemas and produces a structured diff.

```bash
# Compare staging → prod (source is ground truth, target is what you want to fix)
cassachange diff --source-profile staging --target-profile prod

# Compare keyspace against a CQL file
cassachange diff --source-profile prod --source-file ./schema.cql

# JSON output
cassachange diff --source-profile staging --target-profile prod --json

# Only show a specific table
cassachange diff --source-profile staging --target-profile prod --table users
```

Detects: missing tables, extra tables, column additions, column removals, type changes, index additions, index removals.

---

### cassachange-generate

Generates a ready-to-apply V__.cql migration file from a diff.

```bash
# Generate migration to bring prod in line with staging
cassachange generate \
  --source-profile staging \
  --target-profile prod \
  --version 2.0.0

# → migrations/V2.0.0__sync_prod_from_staging.cql
...
cassachange generate \
  ...
  --target-profile prod \
  --version 2.0.0 \
  --output ./migrations/V2.0.0__sync.cql
...
All generated statements use `IF NOT EXISTS` / `IF EXISTS` — idempotent by default.

---

### cassachange-seed

Schema-aware test data seeder. No external dependencies (no faker).

```bash
# Seed all tables, 1000 rows each
cassachange seed --profile dev

# Single table, 100k rows
cassachange seed --profile dev --table user_events --rows 100000

# Larger batches for throughput
cassachange seed --profile dev --rows 50000 --batch-size 100

# Apply TTL to seeded rows
cassachange seed --profile dev --table sessions --ttl 3600

# Preview without writing
cassachange seed --profile dev --dry-run

# Reproducible seed
cassachange seed --profile dev --seed 42

# JSON summary
cassachange seed --profile dev --json

# Also seed counter tables (via UPDATE)
cassachange seed --profile dev --counter
```
...
|---|---|
| `*email*` | `firstname.lastname42@domain.com` |
| `*phone*` / `*mobile*` | `+12125551234` |
| `*status*` / `*state*` | `active` / `pending` / `archived` … |
| `*url*` / `*uri*` | `https://domain.com/api/v1/path` |
| `created_*` / `*_at` | Recent timestamp (last 90 days) |
| `*lat*` / `*latitude*` | `-90.0` to `90.0` |
| `*amount*` / `*price*` | Decimal value |
...
seed_rows: 1000
seed_batch_size: 50
seed_ttl: null          # None = respect table default_ttl
seed_dry_run: false
seed_rng_seed: null
seed_counter: false
seed_json: false
...
### cassachange-dashboard

CI/CD analytics report as a self-contained HTML file.

```bash
# Last 90 days, all keyspaces
cassachange dashboard --profile prod

# 30 days, single keyspace, custom output
cassachange dashboard --profile prod --keyspace myapp --days 30 --output ./reports/dashboard.html
```

**Report contents:**

- 9 KPI cards: total deploys, successful, failed, success rate, scripts run, failures, rollbacks, median deploy time, p95 deploy time
- Daily trend chart (Chart.js bar + line overlay): scripts/day, failures/day, deploys/day
- Recent deploy runs table (20 most recent)
- Keyspace breakdown table
- Slowest scripts table (top 10 by mean execution time)
- Most-failed scripts table (top 10)
- Release tag timeline

Output is a single `.html` file with no external dependencies — commit it to your repo, email it, or open directly in a browser.

Config keys in `cassachange.yml`:

```yaml
dashboard_days: 90
dashboard_output: cassachange-dashboard.html
```

---

### cassachange-contract

Schema contract enforcement — policy-as-code for platform teams.

```bash
# Generate a starter contracts file
cassachange contract init
cassachange contract init --output ./infra/cassachange-contracts.yml

# Check all keyspaces against contracts
cassachange contract check --profile prod

# Check with custom contracts file
cassachange contract check --profile prod --contracts ./infra/cassachange-contracts.yml

# JSON output for CI pipelines
cassachange contract check --profile prod --json

# GitHub Actions inline PR annotations
cassachange contract check --profile prod --github-annotations

# Treat warnings as failures (zero-tolerance mode)
cassachange contract check --profile prod --strict
```

**Exit codes:** `0` = all pass / `1` = warnings / `2` = failures (use as CI gate).

**cassachange-contracts.yml** (generated by `contract init`):

```yaml
version: "1"

# keyspaces:       # optional — list keyspaces to check
#   - myapp_prod

rules:
  # Built-in analyser rules — promote to enforced contracts
  PK001: fail       # low-cardinality partition key
  PK002: fail       # timestamp-only partition key → hotspot
  TH001: fail       # ephemeral table without TTL
  TH004: fail       # unbounded list/map collection
  SA006: fail       # COMPACT STORAGE (deprecated)
  IR001: fail       # too many secondary indexes
  QS001: fail       # ALLOW FILTERING risk
  QS004: skip       # suppressed entirely

  # Custom policy rules
  all_tables_have_ttl:
    action: warn
    exempt: [change_history, audit_log]
    message: "All tables must define a default_time_to_live."
  ...
  partition_key_type:
    action: fail
    allowed: [uuid, timeuuid, text, varchar]
```

**Custom policy rules available:**

| Rule | Enforces |
|---|---|
| `all_tables_have_ttl` | Every table must set `default_time_to_live` |
| `max_secondary_indexes` | Hard cap on index count (`max: 2`) |
| `max_columns` | Hard cap on column count |
| `max_clustering_columns` | Hard cap on clustering column count |
| `require_clustering_column` | Every table must have ≥ 1 clustering column |
| `no_compact_storage` | `COMPACT STORAGE` banned outright |
| `require_comment` | Every table must have `WITH comment = ...` |
| `partition_key_type` | Allowlist for partition key types |
| `no_allow_filtering` | Flag structurally ALLOW FILTERING-dependent tables |
| `table_name_pattern` | Enforce naming convention via regex |
| `column_name_pattern` | Enforce column naming convention via regex |
| `no_counter_mixed` | Counter columns must be in dedicated tables |

All custom rules support `exempt: [table_name, ...]` for known exceptions.

---

### cassachange-capacity

Growth projection engine built on monitor snapshot history.

**Requires:** at least 3 `cassachange monitor` runs (optimal: weekly over 4+ weeks).

```bash
# Project next 180 days
cassachange capacity --profile prod

# 90-day horizon, specific keyspace
cassachange capacity --profile prod --keyspace myapp --horizon 90

# Drill into one table
cassachange capacity --profile prod --table user_events

# Verbose mode (shows R² fit quality, all tables including healthy)
cassachange capacity --profile prod --verbose

# JSON for CI pipelines
cassachange capacity --profile prod --json
cassachange capacity --profile prod --json > capacity-report.json

# Custom thresholds
cassachange capacity --profile prod --warn-partition-mb 30 --crit-partition-mb 80
```
...
|---|---|---|
| `row_count` | 500M rows | 5B rows |
| `partition.mean_size_mb` | 100 MB | 256 MB |
| `partition.estimated_count` | Growth rate only | Growth rate only |
| `ttl.live_row_count` | Rising on TTL table | Rapid rise |
| `collection.avg_size.*` | Collection element growth | Rapid growth |
...
```yaml
capacity_horizon: 180         # days to project forward
capacity_table: null          # restrict to one table
capacity_verbose: false
capacity_json: false
capacity_warn_rows: 500000000
capacity_crit_rows: 5000000000
capacity_warn_partition_mb: 100
capacity_crit_partition_mb: 256
```
...
All tools read from the same `cassachange.yml` and use the same `--profile` flag as core cassachange.
...
| Runtime requirement | Python 3.8+ | JVM (Java 8+) | JVM / Node / Ruby |
| GitHub Actions included | ✔ | ⚠ manual | ⚠ manual |
| live table health snapshots | ✔ enterprise | ✗ | ✗ |
